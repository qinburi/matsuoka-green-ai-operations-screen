...
const chartState = computed(() => analyzing.value ? 'loading' : dataState.value)
...
function chooseTopic(topic: TopicId) {
  ...
  router.replace({ name: 'analysis', query: { topic, source: context.source } })
  runAnalysis()
}
  ...
  <main class="analysis-page">
    ...
    <header class="analysis-header">
      <div class="analysis-brand">
        ...
        <div>
          <span class="eyebrow">MATSUOKA · GREEN AI</span>
          <h1>生产运营诊断中心</h1>
        </div>
      </div>
      ...
      <nav class="analysis-tabs" aria-label="分析主题">
        <button
          ...
          :key="topic"
          type="button"
          :class="{ 'is-active': currentTopicId === topic }"
    ...
    <section class="filter-bar" aria-label="分析筛选条件">
      ...
      <label>
        <span>工厂</span>
        <select v-model="context.factory" @change="updateContext">
          <option v-for="item in contextOptions.factories" :key="item">{{ item }}</option>
        </select>
      </label>
      <label>
        <span>车间 / 产线</span>
        <select v-model="context.line" @change="updateContext">
          <option v-for="item in contextOptions.lines" :key="item">{{ item }}</option>
        </select>
      </label>
      <label>
        <span>时间范围</span>
        <select v-model="context.period" @change="updateContext">
          <option v-for="item in contextOptions.periods" :key="item">{{ item }}</option>
        </select>
      </label>
      <label>
        <span>契约 / 品番</span>
        <select v-model="context.contract" @change="updateContext">
          <option v-for="item in contextOptions.contracts" :key="item">{{ item }}</option>
        </select>
      </label>
      <label>
        <span>演示状态</span>
        <select v-model="dataState" @change="updateDataState">
          <option v-for="(label, key) in stateLabels" :key="key" :value="key">{{ label }}</option>
        </select>
      </label>
      <button class="reanalyze-command" type="button" :disabled="analyzing" @click="runAnalysis">
        {{ analyzing ? '分析中' : '重新分析' }}
      </button>
    </section>
    ...
    <section class="decision-band">
      <div class="metric-strip">
        <article v-for="metric in currentDataset.metrics" :key="metric.id" class="metric-item">
          ...
          <div>
            <strong v-if="dataState === 'normal' || dataState === 'stale'">{{ metric.value.toLocaleString('zh-CN') }}</strong>
      ...
      <aside class="boss-conclusion">
        <div class="boss-label">老板关注结论</div>
        <strong>{{ currentDataset.bossConclusion }}</strong>
        <p><span>建议动作</span>{{ currentDataset.bossAction }}</p>
      </aside>
    </section>
    ...
    <section class="analysis-workspace">
      <article class="analysis-panel problem-panel">
        <header class="panel-heading">
          <div><span>01 · AI INSIGHT MAP</span><h2>{{ primaryChartTitle }}</h2></div>
          <small>{{ currentDataset.description }}</small>
        </header>
      ...
      <article class="analysis-panel issue-panel">
        <header class="panel-heading">
          <div><span>02 · PRIORITY QUEUE</span><h2>问题优先序</h2></div>
          <small>按影响度与置信度排序</small>
        </header>
        <div v-if="dataState === 'normal' || dataState === 'stale'" class="issue-list">
          <button
            v-for="(item, index) in currentDataset.issues"
            :key="item.id"
            type="button"
            :class="[`severity-border--${item.severity}`, { 'is-selected': selectedIssue?.id === item.id }]"
            @click="openIssue(item)"
          >
            <span class="issue-rank">0{{ index + 1 }}</span>
            <span class="issue-copy"><strong>{{ item.shortLabel }}</strong><small>{{ item.stage }}</small></span>
            <span class="issue-impact"><strong>{{ item.impact }}</strong><small>{{ item.impactUnit }}</small></span>
            <span class="issue-confidence">AI {{ Math.round(item.confidence * 100) }}%</span>
          </button>
        </div>
        <div v-else class="issue-state">
          <strong>{{ stateLabels[dataState] }}</strong>
          <span>当前状态下不展示问题结论</span>
        </div>
      ...
      <article class="analysis-panel ranking-panel">
        <header class="panel-heading panel-heading--compact">
          <div><span>03 · IMPACT</span><h2>{{ rankingTitle }}</h2></div>
        </header>
      ...
      <article class="analysis-panel trend-panel">
        <header class="panel-heading panel-heading--compact">
          <div><span>04 · TREND</span><h2>{{ trendTitle }}</h2></div>
        </header>
